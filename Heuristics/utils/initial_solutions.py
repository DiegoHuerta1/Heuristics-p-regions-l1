...
def greedy_algorithm(graph: Graph, num_regions: int) -> dict:
    """
    ...
    # get the matrix with production vectors
    vectors = np.array(graph.vs["x"])

    # define weigths on the edges
    vectors_v = vectors[[v for v,u in graph.get_edgelist()]]
    vectors_u = vectors[[u for v,u in graph.get_edgelist()]]
    graph.es["w"] = np.sum( np.abs(vectors_v - vectors_u), axis=1)
